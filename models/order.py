...
class Orders(Base):
    ...
    amount = Column(Integer)#紀錄訂單總金額

    transaction_id = Column(String)#串接line pay 的時候會用到
    is_pay = Column(Boolean,default=False)#紀錄訂單是否已經付款,預設False代表未付款

    created_time = Column(DateTime,default=func.now())#訂單建立時間

    user_id = Column("user_id",ForeignKey("users.id"))#ForeignKey外來鏈的意思，訂單是由哪一個user建立的
